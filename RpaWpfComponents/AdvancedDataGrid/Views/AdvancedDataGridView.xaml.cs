// ===========================================
// RpaWpfComponents/AdvancedDataGrid/Views/AdvancedDataGridView.xaml.cs - KOMPLETNÁ FUNKČNÁ VERZIA
// ===========================================
...
{
    internal partial class AdvancedDataGridView : UserControl, IDisposable
    {
        private AdvancedDataGridViewModel _viewModel = null!;
        private readonly ILogger<AdvancedDataGridView> _logger;
        private bool _disposed = false;

        // ✅ NOVÉ: Stav pre keyboard shortcuts visibility (lokálne riadenie)
        ...
        public AdvancedDataGridView()
        {
            ...
            // Získaj logger provider z DI alebo použij null provider
            var loggerProvider = GetLoggerProvider();
            _logger = loggerProvider.CreateLogger<AdvancedDataGridView>();
        ...
        #region ✅ NOVÝ: Toggle Keyboard Shortcuts Click Event Handler

        /// <summary>
        /// Click event handler pre toggle klávesových skratiek
        /// </summary>
        private void OnToggleKeyboardShortcuts_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                _logger.LogDebug("Toggle button clicked");
                ...
                // Aktualizuj aj ViewModel ak existuje
                if (_viewModel != null)
                {
            ...
            {
                _logger.LogError(ex, "Error in OnToggleKeyboardShortcuts_Click");
                MessageBox.Show($"Error toggling keyboard shortcuts: {ex.Message}", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }

        /// <summary>
        /// Aktualizuje UI pre keyboard shortcuts visibility
        /// </summary>
        private void UpdateKeyboardShortcutsVisibility()
        {
            try
            {
                // Aktualizuj visibility panelu
                ...
                // Aktualizuj farbu buttonu
                if (ToggleKeyboardShortcutsButton != null)
                {
                    if (_isKeyboardShortcutsVisible)
                    {
                        ToggleKeyboardShortcutsButton.Background = new SolidColorBrush(Color.FromRgb(0, 123, 255)); // #007BFF
                        ToggleKeyboardShortcutsButton.Foreground = Brushes.White;
                        ToggleKeyboardShortcutsButton.BorderBrush = new SolidColorBrush(Color.FromRgb(0, 123, 255));
                    }
                    else
                    {
                        ToggleKeyboardShortcutsButton.Background = new SolidColorBrush(Color.FromRgb(248, 249, 250)); // #F8F9FA
                        ToggleKeyboardShortcutsButton.Foreground = Brushes.Black;
                        ToggleKeyboardShortcutsButton.BorderBrush = new SolidColorBrush(Color.FromRgb(222, 226, 230)); // #DEE2E6
                    }
                }

                _logger.LogDebug("UI updated for keyboard shortcuts visibility: {IsVisible}", _isKeyboardShortcutsVisible);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error updating keyboard shortcuts visibility UI");
            }
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                try
                {
                    _logger?.LogDebug("Disposing AdvancedDataGridView...");
                    ...
                    _logger?.LogInformation("AdvancedDataGridView disposed successfully");
                }
                catch (Exception ex)
                {
                    _logger?.LogError(ex, "Error during AdvancedDataGridView disposal");
                }
            }
        ...
        private void UnsubscribeAllEvents()
        {
            try
            {
                ...
                _logger?.LogDebug("All events unsubscribed successfully");
            }
            catch (Exception ex)
            {
                _logger?.LogError(ex, "Error unsubscribing events during disposal");
            }
        }
        ...
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(AdvancedDataGridView));
        }
        ...
        public AdvancedDataGridViewModel? ViewModel
        {
            ...
            set
            {
                ...
                if (_viewModel != null)
                {
                    ...
                    // Synchronizuj lokálny stav s ViewModelem
        ...
        #region Public Methods - NOVÉ API

        /// <summary>
        /// HLAVNÁ METÓDA: Inicializácia s možnosťou nastavenia počtu riadkov
        /// </summary>
            ...
            int initialRowCount = 100)
        {
            ...
            try
            {
                _logger.LogInformation("Initializing AdvancedDataGridView with {ColumnCount} columns, {InitialRowCount} rows, {TypingDelay}ms throttling",
                    columns?.Count ?? 0, initialRowCount, throttling?.TypingDelayMs ?? 100);
                ...
                await _viewModel.InitializeAsync(columns, validationRules ?? new List<ValidationRuleModel>(), throttling, initialRowCount);
                GenerateDataGridColumns(columns);
                SetupNavigationService();

                _logger.LogInformation("AdvancedDataGridView initialized successfully with {InitialRowCount} rows", initialRowCount);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// BACKWARD COMPATIBILITY: Pôvodná metóda (použije default 100 riadkov)
        /// </summary>
        public async Task InitializeAsync(
            List<ColumnDefinitionModel> columns,
            List<ValidationRuleModel>? validationRules,
            ThrottlingConfiguration throttling)
        {
            await InitializeAsync(columns, validationRules, throttling, 100);
        }

        /// <summary>
        /// SYNCHRONNÁ VERZIA: Pre backward compatibility
        /// </summary>
        public void Initialize(List<ColumnDefinitionModel> columns, List<ValidationRuleModel>? validationRules = null)
        {
            ...
            try
            {
                Task.Run(async () => await InitializeAsync(columns, validationRules, null, 100));
        ...
        public void Reset()
        {
            ...
            try
            {
                _logger.LogInformation("Resetting AdvancedDataGridView (for cleanup)");
                _viewModel?.Reset();
                MainDataGrid.Columns.Clear();

                // Reset keyboard shortcuts state
        ...
        private void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                // Nastavenie počiatočného stavu keyboard shortcuts
                UpdateKeyboardShortcutsVisibility();
